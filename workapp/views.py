...
def signup(request):
    if request.method == 'GET':
        fullname_new = request.GET.get('fullname')
        email_new = request.GET.get('email')
        userType = request.GET.get('usertype')
        password = request.GET.get('password')
        password2 = request.GET.get('repeatpassword')

        if password == password2:
            if User.objects.filter(username=email_new).exists():
                return JsonResponse({"message": 'Email Already In Use'})

            elif fullname_new == "":
                return JsonResponse({"message": 'Full name can not be empty'})

            elif email_new == "":
                return JsonResponse({"message": 'Email can not be empty'})

            elif len(password) < 8:
                return JsonResponse({"message": 'password is too short. Try again'})

            else:
                # create profile
                username = "@" + str(fullname_new.split(" ")[0]) + str(ran.randint(234564, 4567865564))
                user = User.objects.create_user(username=email_new, email=username, password=password,
                                                first_name=fullname_new, last_name="_____")
                user.save()

                profile = Profile(userEmail=email_new, userName=username, userFullName=fullname_new, userType=userType,
                                  userPassword=password)
                profile.save()

                admin = Profile.objects.get(userEmail="workhubagency@outlook.com")
                new_chat = Chat()
                new_chat.save()
                ...
                subject = 'Get Started With WorkHub Community'
                body = message
                send_email(merge_data, email_new, template_name, subject, body)

                login_user = auth.authenticate(username=email_new, password=password)
                if login_user is not None:
                    auth.login(request, login_user)
                    request.session['userId'] = email_new
                    return JsonResponse({"message": "success"})
                else:
                    print("failed to authenticate")
                    request.session['userId'] = email
                    return JsonResponse({"message": "Failed to authenticate"})
...
def login_authenticate(request):
    if request.method == "GET":
        ...
        if user is not None:
            ...
            return JsonResponse({"message": 'Credentials Invalid'})
    else:
        pass
...
def employee_dashboard(request):
    ...
    return_single = "no"
    if request.session.get('job_unique_call_value') == "active":
        return_single = "yes"
        request.session['job_unique_call_value'] = "not valid"

    job_id = request.session.get('job_unique_call_id')
    if profile.userType == "employer":
        return redirect('/employer_dashboard')
    else:
        pass
    return render(request, "employee_dashboard.html", {"return_single": return_single, "job_id": job_id})
...
def employee_offers_offline(request):
    jobs = []
    num_of_jobs = 0
...
def employee_all_offers(request):
    jobs = []
    num_of_jobs = 0
...
def employee_filter_offers_offline(request):
    jobs = []
    num_of_jobs = 0
...
def employee_single_offer_offline(request):
    job = None
    if request.method == 'GET':
        id_value = request.GET.get('job_id')
        x = JobPost.objects.get(id=id_value)
...
def employee_application_info(request):
    if not request.user.is_authenticated:
        return redirect("/login")
    else:
        pass
    ...
    return JsonResponse({"profile_email": profile.userEmail, "profile_resume": profile_resume, "profile_bio": profile.userBio,
    ...
    if request.method == 'GET':
        ...
        if x.job_external == "true":
            ...
            if Application.objects.filter(job=x, job_applicant=profile):
                ...
                newApplication = Application(job=x, job_applicant=profile, application_status="pending",
                                             interview_date="", cover_letter=cover_letter,
...
def employee_process_job_application_external(request):
    if not request.user.is_authenticated:
        return redirect("/login")
    else:
        pass
    ...
    if request.method == 'GET':
        ...
        message = "betta"
    if message == "true" or message == "external":
        pass
    else:
        request.session['job_unique_call_id'] = request.GET.get('job_id')
        request.session['job_unique_call_value'] = "active"
...
def employee_private_chat(request):
    ...
    if str(friend_profile_email) == "workhubagency@outlook.com":
    ...
    return JsonResponse({"user_profile_name": profile_name, "user_profile_pic": profile_img, "friend_profile_pic":
        friend_profile_img, "friend_profile_id": friend_profile_id, "friend_profile_name": friend_profile_name, "friend_verification_email":
...
def create_roles(request):
    ...
    if request.method == "GET":
        ...
        if input_list is None:
            ...
            out_file = open("./template/Sitemap.xml", "r+")
            lines = out_file.readlines()
            out_file.close()
            writeFile = open("./template/Sitemap.xml", 'w')
            for line in lines[:-1]:
                writeFile.writelines(line)
            writeFile.writelines("<url>")
            writeFile.writelines(f"\n<loc>https://workhubagency.pythonanywhere.com/jobview/{new_job.id}</loc>")
            writeFile.writelines("\n<lastmod>2023-12-11T09:18:45+00:00</lastmod>")
...
def employer_profile(request):
    ...
    return JsonResponse({"profile_email": profile.userEmail, "profile_bio": profile.userBio, "profile_bio_length":
        len(str(profile.userBio)), "profile_contact": profile.userContact, "profile_name":
                             profile.userFullName, "profile_title": profile.userTitle,
                         "profile_gender": profile.userGender,
                         "profile_location": profile.userLocation, "profile_birth_date": profile.userDOB,
...
def viewprofile(request, user_id):
    ...
    return render(request, "viewprofile.html", {"user_id": user_id, "userType": profile.userType})


def cover_letter_viewer(request, application_id):
    application = Application.objects.get(id=application_id)
    cover_letter = application.cover_letter
    return render(request, "cover_letter_viewer.html", {"cover_letter": cover_letter})
...
def get_profile(request):
    ...
    return JsonResponse({"profile_email": profile.userEmail, "profile_bio": profile.userBio,
                         ...
                         "experience_message": experience_message, "skills": skills, "skill_message": skill_message,
                         "profile_picture": profile_picture, "profile_gender": profile.userGender,
                         "profile_location": profile.userLocation, "profile_type": profile.userType, "profile_birth_date": profile.userDOB, })
...
def jobview(request, job_id):
    ...
    if request.user.is_authenticated:
        if not request.session.get('userId', None):
            ...
            profile = Profile.objects.get(userEmail=request.session.get('userId'))
            if profile.userType == "employee":
                user_is_authenticated = "yes"
                request.session['job_unique_call_id'] = job_id
                request.session['job_unique_call_value'] = "active"
                return redirect("/dashboard")
            else:
                pass
    ...
    job_des = ""
    if len(str(job.job_description)) > 50:
        job_des = str(job.job_description)[:49] + "..."
    else:
        job_des = str(job.job_description)
    return render(request, "job_view.html", {"job_id": job_id, "user_is_authenticated": user_is_authenticated, "job_title": job.job_title, "job_des": job_des})
...
def unsubscribe(request, unique_id):
    return HttpResponse("<h2>Unsubscribe Successfully</h2>")


def send_email(data, user_email, mail_template, email_subject, email_body):
    html_body = render_to_string(str(mail_template), data)
    message = EmailMultiAlternatives(
        subject=email_subject,
        body=email_body,
        from_email='uniwylimp@gmail.com',
        to=[str(user_email)]
    )
